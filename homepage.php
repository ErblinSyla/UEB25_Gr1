...
<html lang="en">
    <head>
        ...
        <style>
            ...
            #instructors-title,#hands-title,#flexible-title {
    ...
    if (audio) {
        // Convert the NodeList to an array and use map() to add event listeners
        ...
        <!--                                               MAIN SECTION                                          -->
            ...
            <div class="text-admission">
               ...
               <audio id="admission-click-audio" src="audio/shift-page.mp3"></audio>
                ...
                <a href="admission.html" class="cta-button">Admissions</a>
            </div>
            </section>
            <script>
...
document.addEventListener("DOMContentLoaded", () => {
    setTimeout(() => {
        ...
        section.style.opacity = '1'; // Fade in the section after 1 second
...
<!--                                                 FLEXIBLE LEARNING SECTION                                   -->
    ...
    <script>
        ...
        // Check if the name input contains a number (using isNaN)
        if (isNaN(nameInput.value) === false || nameInput.value.trim() === "") {
            // Prevent form submission if it's a number or empty
            event.preventDefault();  
            nameError.style.display = "block";  // Show the error message
        } else {
            nameError.style.display = "none";  // Hide error message if input is valid
        }
    });
       document.getElementById('newsletterForm').addEventListener('submit', function(event) {
    ...
    if (element && audio) {
        element.addEventListener('mouseenter', () => {
            try {
                ...
                // Catch any error that occurs during the audio play attempt
...
function addHoverAudioEffect(elementSelector, audioId) {
    ...
    document.getElementById('newsletterForm').addEventListener('submit', function(event) {
    const nameInput = document.getElementById('name-input');
    // Example: remove any extra spaces before submitting
...
<script>
    ...
    // Display the month and year in the footer
